...
public class Main {
    ...
    static ArrayList<Integer> orderResult;
    static int[] path;
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println(orderResult.size());
        for (int i : orderResult) {
            System.out.print(i + " ");
        }
    }
    ...
    static void dk(int departure, int arrival) {
        ...
        while (!pq.isEmpty()) {
            ...
            if (nowD == arrival) {
                if (result > distance[nowD]) {
                    result = distance[nowD];
                    orderResult = new ArrayList<>();
                    int temp = nowD;
                    while (temp != departure) {
                        orderResult.add(temp);
                        temp = path[temp];
                    }
                    orderResult.add(departure);
                    Collections.reverse(orderResult);
                }
            }
